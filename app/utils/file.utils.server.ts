...
export function getFileInfos(base64: string, baseName: string): [base64: string, fileName: string, previewFileName: string, thumbnailFileName: string, ext: string] {
  let base64Data = base64.split(';base64,').pop() ?? ''
  let fileExt = base64GetExtRegex.exec(base64)?.[1]

  return [base64Data, `${baseName}.${fileExt}`, `${baseName}-preview.${fileExt}`, `${baseName}-thumbnail.${fileExt}`, fileExt ?? 'jpg']
}
...
export async function saveImage(base64: string, entryId: string) {
  ...
  if (base64Data && fileName) {
    ...
    await Promise.all([
      pictures.upload(fileName, await fs.readFile(fullPath), { contentType: `image/${ext}`}),
      pictures.upload(previewFileName, await fs.readFile(fullPreviewPath), { contentType: `image/${ext}`}),
      pictures.upload(thumbnailFileName, await fs.readFile(fullThumbailPath), { contentType: `image/${ext}`}),
    ])
